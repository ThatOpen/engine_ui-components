...
export class TableRow<T extends TableRowData> extends LitElement {
  ...
  private onSelectionChange(e: Event) {
    ...
    if (target.value) {
      if (
        !this.isHeader &&
        this._isRangeClick &&
        this.table._lastSelectedData
      ) {
        const flat = Table.flattenData<T>(this.table.value).map(
          (entry) => entry.data,
        );
        const anchorIdx = flat.indexOf(this.table._lastSelectedData);
        const currentIdx = flat.indexOf(this.data);
        ...
        if (anchorIdx !== -1 && currentIdx !== -1) {
          ...
          this.table.selection.add(...flat.slice(start, end + 1));
        }
      } else {
        let data = [this.data];
        if (this.isHeader) {
        ...
        if (!this.isHeader) {
          this.table._lastSelectedData = this.data;
        }
      }
    } else {
      if (this.isHeader) {
  ...
  protected render() {
    ...
    if (this.groupData?._isComputedGroup) {
      const visibleColumns = this.table.dataKeys.filter(
    ...
    for (const column in data) {
      if (
        !this.groupData?._isComputedGroup &&
        this.hiddenColumns.includes(column)
      )
        continue;
