...
const model = await ifcLoader.load(typedArray, true, "small");
...
const [propertiesTable, updatePropertiesTable] = CUI.tables.entityEditing({
  components,
  modelIdMap: {},
});
...
highlighter.events.select.onHighlight.add((fragmentIdMap) => {
  updatePropertiesTable({ modelIdMap: fragments.getModelIdMap(fragmentIdMap) });
});
