...
export interface TopicPanelUI {
  ...
  onUpdateInformation?: (topic: OBC.Topic) => void | Promise<void>;
...
>((state: NewCommentUI) => {
  ...
  if (components) {
    ...
    const filteredTopics = [...bcfTopics.list.values()].filter(
      (t) => t !== topic,
    );
    topics = topicsList({
      components,
...
export const topicDataTemplate = (state: TopicPanelUI) => {
  const {
    components,
    topic,
    styles,
