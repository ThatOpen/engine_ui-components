...
export class UIManager<GridArea extends string = Area> {
  private _viewportResizeObserver = new ResizeObserver(
  ...
  private updateGridContainers(grid: Grid) {
    ...
    for (const row of grid.rows) {
      ...
      for (const area of areas) {
        const panelsContainer = this.containers.panels.find(
          (container) => container.style.gridArea === area,
        );
        const toolbarsContainer = this.containers.toolbars.find(
          (container) => container.style.gridArea === area,
        );
        if (panelsContainer) {
  ...
  getPanelsContainer(area: GridArea) {
    ...
    const container = panelContainers.find(
      (container) => container.gridArea === area,
    );
    if (!container) {
      throw new Error(
        `UIManager: ${UIManager.config.panelsContainerPrefix}${area} wasn't define in --bim-grid--tpl`,
      );
    }
    return container;
  }
  ...
  getToolbarsContainer(area: GridArea) {
    ...
    if (!container) {
      throw new Error(
        `UIManager: ${UIManager.config.toolbarsContainerPrefix}${area} wasn't define in --bim-grid--tpl`,
      );
    }
    return container;
  }
