import{x as d}from"./lit-element-BOuWA2QR.js";import{P as p,C as m,W as u,S as f,i as b,b as w,G as g,j as h,I as x,T as y}from"./index-CTMKzHCn.js";import{t as v}from"./index-BCyzZAG-.js";p.init();const a=document.createElement("bim-viewport"),t=new m,C=t.get(u),e=C.create();e.name="Default World";e.scene=new f(t);e.scene.three.background=null;e.scene.setup();e.renderer=new b(t,a);const{postproduction:o}=e.renderer;e.camera=new w(t);e.camera.controls.setLookAt(1.5,1.4,.12,-3.5,-.5,-7);a.addEventListener("resize",()=>{e.renderer&&e.renderer.resize(),e.camera.updateAspect()});t.init();const S=t.get(g),i=S.create(e);i.material.uniforms.uColor.value=new h("#4D4D4D");const l=t.get(x);await l.setup();const A=await fetch("https://thatopen.github.io/engine_ui-components/resources/small.ifc"),P=await A.arrayBuffer(),W=new Uint8Array(P),D=await l.load(W);e.scene.three.add(D);o.enabled=!0;o.customEffects.excludedMeshes.push(i.three);o.setPasses({ao:!0});const[n]=v.worldsConfiguration({components:t}),E=y.create(()=>d`
