import{n as k,b as e,o as c}from"./when-D9oPOCfO.js";import{M as T,C as o}from"./index-CICXNLO4.js";import{i as y}from"./custom-icons-BJvlg01G.js";T.init();T.addIconsCollection({...y});const d=()=>e`
...
`,C=t=>e`
    <bim-panel-section label="Panel Section" icon="solar:settings-bold">
      ...
      <bim-dropdown search-box>
        ${Array.from({length:100},(b,p)=>e`<bim-option style="width: 12rem;" label="Option ${p+1}"></bim-option>`)}
      </bim-dropdown>
  ...
  `,u=o.create(()=>{let t;return e`
    <bim-panel-section label="Assignments" fixed style="height: unset">
      ...
      <bim-table selectable-rows @rowcreated=${({detail:{row:s}})=>{s.style.borderBottom="1px solid var(--bim-ui_bg-contrast-60)",s.addEventListener("click",()=>console.log("hi there"))}} ${k(s=>{if(!s)return;t=s;const m={padding:"0.25rem 0.375rem",borderRadius:"0.25rem"};t.dataTransform={Status:i=>{if(typeof i!="string")return i;if(i==="In Progress"){const n={...m,backgroundColor:"#3c59c3",color:"white"};return e`<bim-label style=${c(n)}>
            ${i}
            ${o.create(d)}
          </bim-label>`}if(i==="Pending"){const n={...m,backgroundColor:"#5c5c5c",color:"white"};return e`<bim-label style=${c(n)}>${i}</bim-label>`}if(i==="Completed"){const n={...m,backgroundColor:"#3a7829",color:"white"};return e`<bim-label style=${c(n)}>${i}</bim-label>`}if(i==="Scheduled"){const n={...m,backgroundColor:"#9e2980",color:"white"};return e`<bim-label style=${c(n)}>${i}</bim-label>`}return i},AssignedTo:(i,n)=>e`
  ...
  `}),v=t=>e`
    <bim-toolbar>
      ...
      <bim-toolbar-section label="Some section">
        ...
        <bim-button active @click=${({target:a})=>{const g=u.style.display==="none";a.active=g,g?u.style.removeProperty("display"):u.style.display="none"}} label="Table" vertical icon="material-symbols:table"></bim-button>
        <bim-toolbar-group>
  ...
  `),b=document.createElement("bim-grid");return b.floating=!0,b.layouts={main:{template:`
      ...
      /1fr
    `,elements:{bottomToolbar:t,rightToolbar:r}}},b.layout="main",e`
    <bim-viewport>${b}</bim-viewport>
  `}),l=document.body.querySelector("bim-grid");l.elements={header:(()=>{const t=document.createElement("div");return t.style.backgroundColor="#641b1b66",t})(),sidebar:(()=>{const t=document.createElement("div");return t.style.backgroundColor="#1b536466",t})(),content:(()=>{const t=document.createElement("div");return t.style.backgroundColor="#1b644c66",t})(),toolbarA:{template:v,initialState:{},label:"Toolbar A"},toolbarB:{template:A,initialState:{},label:"Toolbar B"},leftPanelSettings:{template:C,initialState:{},label:"Settings"},leftPanelProperties:{template:S,initialState:{},label:"Properties"},viewport:w,bottomPanel:u,rightPanelControls:x,rightPanelInfo:P};const h="tabs:ribbon(toolbarA,toolbarB)",B="tabs:left(leftPanelSettings,leftPanelProperties)",$="panel:right(rightPanelControls,rightPanelInfo)";l.areaGroups={left:{switchersCompact:!0},right:{label:"Inspector"}};l.layouts={main:{template:`
      "header header" 80px
      "sidebar content" 1fr
      / 80px 1fr
    `},app:{template:`
      "${h} ${h} ${h}" max-content
      "${B} viewport ${$}" minmax(0, 1fr)
      "bottomPanel bottomPanel bottomPanel" minmax(0, 20rem)
      / 15rem 1fr 15rem
    `}};const D=document.body.querySelector("bim-button");D&&D.addEventListener("click",()=>{const{layout:t}=l;switch(t){case void 0:l.layout="main";break;case"main":l.layout="app";break;case"app":l.layout=void 0;break;default:console.log("No follow up action")}});
