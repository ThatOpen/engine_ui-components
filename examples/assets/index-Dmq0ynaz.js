import{m as u,W as w,F as b,I as h,R as g}from"./index-DMl8TH7x.js";const k=o=>{const{components:e,modelUserData:n,worldName:i}=o;return u`
    <bim-button
      data-ui-id="import-ifc"
      label="Load IFC"
      icon="mage:box-3d-fill"
      @click=${()=>{if(!(e&&i))return;const r=[...e.get(w).list.values()].find(c=>"name"in c&&c.name===i);if(!r)return;const t=document.createElement("input");t.type="file",t.accept=".ifc",t.onchange=async()=>{if(t.files===null||t.files.length===0)return;const c=t.files[0],l=await c.arrayBuffer(),s=new Uint8Array(l),p=c.name.replace(".ifc",""),y=e.get(b),f=e.get(h);f.settings.autoSetWasm=!1,f.settings.wasm={path:"https://unpkg.com/web-ifc@0.0.70/",absolute:!1};const m=await f.load(s,!0,p,{userData:n});r.scene.three.add(m.object),m.useCamera(r.camera.three),y.core.update(!0)},t.click()}}
    ></bim-button>
