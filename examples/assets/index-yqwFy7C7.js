import{e as F,c as u,V as _,i as x,F as $,f as S,g as U,j as G,B,u as H}from"./index-Dn97DhiK.js";import"./lit-html-CgQwCkHV.js";const N=(n,s)=>{const o=s[n],e=(o==null?void 0:o.name)??n,i=e.trim().split(/\s+/);let t,l;return i[0]&&i[0][0]&&(t=i[0][0].toUpperCase(),i[0][1]&&(l=i[0][1].toUpperCase())),i[1]&&i[1][0]&&(l=i[1][0].toUpperCase()),u`
  ...
  `},ne=(n,s)=>{const{components:o,actions:e,metaDataTags:i}=n,t=o.get($),l=(e==null?void 0:e.dispose)??!0,c=(e==null?void 0:e.download)??!0,r=(e==null?void 0:e.visibility)??!0,a=i??[];s.hiddenColumns=["modelId","metadata"],s.headersHidden=!0,s.noIndentation=!0,s.dataTransform={Name:(d,m)=>{if(!t.initialized)return d;const{modelId:b,metadata:g}=m;if(!b)return d;const p=t.list.get(b);if(!p)return b;const h=[];if(g){const f=JSON.parse(g);for(const y of a){const v=f[y];if(!(typeof v=="string"||typeof v=="boolean"||typeof v=="number"))continue;const I=u`
            ...
            `;h.push(I)}}let w;l&&(w=u`<bim-button @click=${()=>t.core.disposeModel(p.modelId)} icon="mdi:delete"></bim-button>`);let T;r&&(T=u`<bim-button @click=${async({target:y})=>{y.loading=!0,await p.setVisible(void 0,y.hasAttribute("data-model-hidden")),await t.core.update(!0),y.toggleAttribute("data-model-hidden"),y.icon=y.hasAttribute("data-model-hidden")?"mdi:eye-off":"mdi:eye",y.loading=!1}} icon="mdi:eye"></bim-button>`);let C;return c&&(C=u`<bim-button @click=${async()=>{const y=await p.getBuffer(!1),v=new File([y],`${p.modelId}.frag`),I=document.createElement("a");I.href=URL.createObjectURL(v),I.download=v.name,I.click(),URL.revokeObjectURL(I.href)}} icon="flowbite:download-solid"></bim-button>`),u`
  ...
  `},ae=(n,s)=>{const{components:o,topic:e}=n;s.noIndentation=!0,s.headersHidden=!0,s.hiddenColumns=["Guid"],s.columns=["Title",{name:"Actions",width:"auto"}];const i={selectComponents:!0,colorizeComponent:!0,resetColors:!0,updateCamera:!0,delete:!0,unlink:!!e,...n.actions},t=o.get(U);s.dataTransform={Actions:(l,c)=>{const{Guid:r}=c;if(!(r&&typeof r=="string"))return l;const a=t.list.get(r);if(!a)return l;const d=async({target:C})=>{C.loading=!0,await a.go(),C.loading=!1};let m;i.selectComponents&&(m=u`
          <bim-button label="Select Components" @click=${async({target:f})=>{const y=o.get($),v=o.get(x);if(!v.isSetup)return;f.loading=!0;const I=await y.guidsToModelIdMap([...a.selectionComponents]);await v.highlightByID("select",I),f.loading=!1}}></bim-button>
        `);let b;i.colorizeComponent&&(b=u`
          <bim-button label="Colorize Components" @click=${async({target:f})=>{f.loading=!0,await a.setColorizationState(!0),f.loading=!1}}></bim-button>
        `);let g;i.resetColors&&(g=u`
          <bim-button label="Reset Colors" @click=${async({target:f})=>{f.loading=!0,await a.setColorizationState(!1),f.loading=!1}}></bim-button>
  ...
  `},ue=(n,s)=>{const{topic:o,styles:e}=n,i={delete:!0,...n.actions};s.headersHidden=!0,s.hiddenColumns=["guid","author"],s.dataTransform={Comment:(t,l)=>{const{guid:c}=l;if(typeof c!="string")return t;const r=o.comments.get(c);if(!r)return t;const a=()=>{o.comments.delete(c)};let d;if(i.delete){const m=`btn-${H.newRandomId()}`;d=u`
          <div>
            <style>
              #${m} {
      ...
      `}}},be=(n,s=!0)=>{const o=S.create(V,n),[e,i]=o;if(ue(n,e),s){const{topic:t}=n,l=()=>i();t.comments.onItemSet.add(l),t.comments.onItemUpdated.add(l),t.comments.onItemDeleted.add(l),t.comments.onCleared.add(l)}return o},ge=Object.freeze(Object.defineProperty({__proto__:null,commentsList:be,commentsListTemplate:V},Symbol.toStringTag,{value:"Module"})),ye={...K,...oe,...se,...re,...me,...ge};export{N as a,M as b,be as c,k as d,ye as e,de as t,le as v};
